...
def main(args):
    ...
    for epoch in range(epochs):
        ...
        for split in splits:
            ...
            data_loader = DataLoader(dataset=datasets[split], batch_size=32, shuffle=(split == 'train'),
                                     num_workers=cpu_count(),pin_memory=torch.cuda.is_available())
            ...
            for iteration, batch in enumerate(data_loader):
                ...
                if split == 'train':
                    ...
                    fader_opt.step()
                    step += 1
                if iteration % 1000 == 0 and split == 'train':
                    print(
                        '\nPhase: {}| Iteration {}/{}  |  | Fader Loss: {:.4f} | VAE Loss: {:.4f} | L1_loss: {:.4f} | KL Weight: {:.4f} || KL Loss: , {:.4f}| '
                        .format(split.upper(), iteration, len(data_loader), fader_loss, vae_loss, l1_loss, KL_weight,
                                KL_loss / batch_size), flush=True)

                if iteration % 1000 == 0 and split == 'valid':
                    print(
                        '\nPhase: {}| Iteration {}/{}  || Fader Loss: {:.4f} | VAE Loss: {:.4f} |L1 loss: {:.4f}| KL_Weight; {:.4f} || KL Loss: {:.4f} |'
                        .format(split.upper(), iteration, len(data_loader),  fader_loss, vae_loss, l1_loss, KL_weight,
                                KL_loss / batch_size), flush=True)
            ...
            fader_loss_summary = restart(fader_loss_summary, split)
            if split == "valid":

                #if not(os.path.exists("Sentiment/Latent")):
                    #os.mkdir("Sentiment/Latent")
                z_dataset = pd.DataFrame(z_dataset)
                final = pd.DataFrame({'Label': labels})
                print(f"z {z_dataset.shape}")
                final.to_csv("Sentiment/Latent/" + str(epoch) + "_label.csv", index=False)
                z_dataset.to_csv("Sentiment/Latent/" + str(epoch) + ".csv", index=False)

            if split == 'valid' and args.save_model:
            ...
            print("\nEpoch: {} | Fader Loss: {:.4f} ".format(epoch,fader_loss_summary[split + "_epoch"][epoch]))
